CREATE TABLE `ps_map_data_source_gungames` (
...
CREATE TABLE `ps_plr_data_source_gungames` (
  `dataid` int unsigned NOT NULL default '0',
  `ctkills` smallint unsigned NOT NULL default '0',
  `terroristkills` smallint unsigned NOT NULL default '0',
...
CREATE TABLE `ps_plr_maps_source_gungames` (
  `dataid` int unsigned NOT NULL default '0',
  `ctkills` smallint unsigned NOT NULL default '0',
  `terroristkills` smallint unsigned NOT NULL default '0',
  `ctdeaths` smallint unsigned NOT NULL default '0',
...
INSERT INTO `ps_config_events` (`id`, `gametype`, `modtype`, `eventname`, `alias`, `regex`, `idx`, `ignore`, `codefile`) 
    VALUES 
        ...
        (26,'source','gungames','cs_teamscore','','/^Team \"([^\"]+)\" scored \"([^\"]+)\" with \"([^\"]+)\" players/',260,0,''),
        (27,'source','gungames','teamtrigger','','/^Team \"([^\"]+)\" triggered \"([^\"]+)\"(.*)/',270,0,''),
        (28,'source','gungames','cs_ignore1','','/^Counter-Strike Plugin/',280,1,''),
        (29,'source','gungames','cs_ignore2','','/^Votemap/',290,1,''),
...
INSERT INTO `ps_config_plrbonuses` (`id`, `eventname`, `enactor`, `enactor_team`, `victim`, `victim_team`, `description`, `gametype`, `modtype`) 
    VALUES 
        (1,'planted_the_bomb',2,1,0,-1,'When a player plants the bomb','source','gungames'),
        (2,'defused_the_bomb',2,0,-1,0,'A player defused the bomb successfully. Use the \'bomb_defused\' event for team bonses.','source','gungames'),
        (3,'rescued_a_hostage',3,0,0,0,'When a player rescues a hostage','source','gungames'),
        (8,'bomb_runner',1,0,0,0,'Player spawned with and planted the bomb w/o dropping it','source','gungames'),
        (7,'bomb_defused',0,2,0,-2,'TEAM event that fires when a round ends and the CT team defuses the bomb (use \'defused_the_bomb\' for enactor bonus)','source','gungames'),
        (10,'target_bombed',5,2,0,-2,'TEAM event that fires when the bomb explodes and the terrorists win','source','gungames'),
        (12,'all_hostages_rescued',0,2,0,-2,'TEAM event that fires when all hostages on a CS map have been rescued (no enactor)','source','gungames'),
        (13,'target_saved',0,5,0,-5,'TEAM event that fires when a round ends and the terrorists did not plant a bomb','source','gungames'),
        (14,'hostages_not_rescued',0,2,0,-2,'TEAM event that fires when a round ends and the hostages are NOT rescued','source','gungames'),
        (28,'killed_a_hostage',-3,0,0,0,'When a player kills a hostage','source','gungames'),
        (29,'escaped_as_vip',10,0,0,0,'When a player escapes an \'es\' map as the VIP (use \'vip_escaped\' for team bonuses)','source','gungames'),
        (30,'vip_escaped',0,5,0,-5,'TEAM event that fires when the VIP escapes the map (use escaped_as_vip for enactor bonus)','source','gungames'),
        (31,'assassinated_the_vip',10,0,-5,0,'When a player kills the VIP (use \'vip_assassinated\' for team bonuses)','source','gungames'),
        (32,'vip_assassinated',0,10,0,-10,'TEAM event that fires when the VIP was killed (use \'assassinated_the_vip\' for enactor bonus)','source','gungames'),
        (33,'vip_not_escaped',0,5,0,-5,'TEAM event that fires when the VIP does not escape and terrorists win','source','gungames'),
...
INSERT INTO `ps_config_awards` (`id`, `enabled`, `idx`, `type`, `negative`, `class`, `name`, `groupname`, `phrase`, `expr`, `order`, `where`, `limit`, `format`, `gametype`, `modtype`, `rankedonly`, `description`) 
    VALUES 
        ...
        (5,1,90,'player',0,'','Most Bombings','','{$player.link} has the most successful bombings ({$award.value} bombs)','{$bombexploded}','desc','',25,'commify','source','gungames',1,'Players with the most successful bombed targets'),
        ...
        (7,1,110,'player',0,'','Most Hostages Rescued','','{$player.link} has rescued the most hostages ({$award.value})','{$rescuedhostages}','desc','',0,'commify','source','gungames',1,'Players that risk life and limb to save hostages!'),
        ...
        (11,1,80,'player',0,'','Demolitions Man','','{$player.link} has the best demolition percentage of {$award.value}','{$bombrunner} / {$bombexploded} * 100','desc','',25,'%0.02f%%','source','gungames',1,'Players with the highest percentage of spawning with and planting the bomb and actually having it explode.'),
        (12,1,100,'player',0,'','Bomb Defusal Expert','','{$player.link} has the best defusal rating of {$award.value}','{$bombdefused} / {$bombdefuseattempts} * 100','desc','{$bombdefused} >= 5',25,'%0.02f%%','source','gungames',1,'Player with the best bomb defuses percentage (must have at least 5 defuses to count)'),
        ...
        (26,1,120,'player',1,'','Most Hostages Killed','','{$player.link} has killed the most hostages ({$award.value})','{$killedhostages}','desc','',0,'%s','source','gungames',1,'Players that like to kill hostages instead of rescuing them.');
