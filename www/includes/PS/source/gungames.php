 ...
 *
 *	Version: $Id: gungames.php  2023-07-01 $
 *
 *	$Id$
 *
 *	PS::source::gungames
 *	Source::gungames mod support for PsychoStats front-end
 */
if (!defined("PSYCHOSTATS_PAGE")) die("Unauthorized access to " . basename(__FILE__));
if (defined("CLASS_PS_SOURCE_GUNGAMES_PHP")) return 1;
define("CLASS_PS_SOURCE_GUNGAMES_PHP", 1);

include_once(rtrim(dirname(__DIR__), '/\\') . '/source/cstrikes.php');

class PS_source_gungames extends PS_source_cstrikes {

var $class = 'PS::source::gungames';

// do not declare the variables array since we're using the existing array
// from the cstrikes subclass.
//var $CLAN_MODTYPES = array();

function __construct(&$db) {
	parent::PS_source_cstrikes($db);
...
function PS_source_gungames(&$db) {
    self::__construct($db);
}
