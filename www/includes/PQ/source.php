...
/**
	This PsychoQuery::SOURCE class will automatically determine if the server being queried is HL1 or HL2.
...
if (defined("CLASS_PQ_SOURCE_PHP")) return 1;
define("CLASS_PQ_SOURCE_PHP", 1);
...
function init() {
	...
	$this->source_version = 0;	// 0 = unknown version
...
// sets and returns the source version to properly decode packets
function _hlver($header=NULL) {
	...
	if ($header === NULL and $this->source_version) {
	...
	if ($header === NULL) {		// if header is still null, we have no packet bytes to check
		$this->source_version = 0;		
	} else {
		if ($this->DEBUG) print "DEBUG: Determing source version from header packet:\n" . $this->hexdump(substr($header,0,16));
		$code = @substr($header, 4, 1);
		if (!$code) {
			$this->source_version = 0;
		} elseif (in_array($code, array('m', 'o'))) {
			$this->source_version = 1;
		} else {
			$this->source_version = 2;
		}
	}
	return $this->source_version;
}
...
function query_info($ip=NULL) {
	...
	if ($this->raw != '') {
		...
		if ($ver == 1) {
			return $this->_parse_info_source1();
		} elseif ($ver == 2) {
			return $this->_parse_info_source2();
		} else {
...
// issues an RCON command to a source version 2 server (source)
...
// internal function to send a non-authoritative query to a source server (NOT RCON COMMANDS)
...
// internal function to send a query to the master source servers (steam) for an IP list.
